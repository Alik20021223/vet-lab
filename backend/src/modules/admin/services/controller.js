...
export async function getById(request, reply) {
  try {
    ...
    if (!item) return error(reply, 'Услуга не найдена', 404);
...
export async function create(request, reply) {
  try {
    ...
    if (rawData.image && rawData.image.file) {
      const result = await saveImage(rawData.image, 'lab-services');
...
export async function remove(request, reply) {
  try {
    ...
    return success(reply, { message: 'Услуга успешно удалена' });
  } catch (err) {
    return error(reply, err.message, 500);
  }
}
